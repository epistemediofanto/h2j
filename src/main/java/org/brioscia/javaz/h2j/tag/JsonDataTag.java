...
public class JsonDataTag extends BaseTag {
	...
	public void processNode(XmlProcessor processor, Node node) throws H2JFilterException {
		...
		assertNotNull(nodeName, "missing attribute name in tag jsondata");
		nodeNameValue = nodeName.getNodeValue();
		assertNotNull(nodeNameValue, "empty attribute name in tag jsondata");

		nodeValue = attributes.getNamedItem("value");
		assertNotNull(nodeValue, "missing attribute value in tag jsondata");
		nodeValueValue = nodeValue.getNodeValue();
		assertNotNull(nodeValueValue, "empty attribute value in tag jsondata");
		...
		newNode.setTextContent(//
				"var jsonMap = new Map();" + //
						"jsonMap['" + nodeNameValue + "'] = JSON.parse('" + jsonString + "');");
